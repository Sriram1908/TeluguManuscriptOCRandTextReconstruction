...
# ── 1. Configuration (Fixed Project Root) ───────────
# Points to .../Synthetic_Data/MyScriptsss
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__)) 
# Points to .../Synthetic_Data
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR) 

OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Type_2_Output")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Corrected paths for your Garuda Puranam source
...
# ── 2. Helper Functions (No Logic Changes) ──────────
    ...
    while word_idx < len(all_words):
        ...
        while word_idx < len(all_words):
            ...
            for c in clusters:
                ...
                if not img: img = generate_synthetic(c, syn_font, FIXED_HEIGHT)
                if img:
                    word_imgs.append(img); word_w += img.size[0] + CHAR_SPACING

            # SEQUENCE SHIELD: If word doesn't fit, break and save it for the next image
            if curr_x + word_w > (bg_rgb.shape[1] - RIGHT_MARGIN):
                break 
            
            # If it fits, paste the whole word
            ...
            curr_x += WORD_SPACING
            word_idx += 1 # Advance sequence only after success

        name = f"type2_baseline_{img_count:05d}"
        ...
        img_count += 1
        if img_count >= 2000: break # Set to your Bucket 1 target

    print(f"✅ Type 2 (Baseline) complete. Check {OUTPUT_DIR}")