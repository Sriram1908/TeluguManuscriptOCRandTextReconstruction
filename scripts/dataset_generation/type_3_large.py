...
# This points to .../Synthetic_Data/MyScriptsss
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__)) 
# This points to .../Synthetic_Data (Your Project Root)
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR) 

OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Type_3_Large_Output")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ── 2. Verified Paths ───────────────────────────────
TEXT_SOURCES = [os.path.join(PROJECT_ROOT, "corpus", "Ramayanam", "ram.txt")]
...
def apply_large_solid_stains(canvas, text_limit_x):
    """Adds solid white LARGE stains that block multiple words."""
    draw = ImageDraw.Draw(canvas)
    # 1. Large Strips (Increased range for heavier damage)
    for _ in range(random.randint(2, 3)):
        tx = random.randint(X_MARGIN + 100, text_limit_x - 100)
        cy = random.randint(20, 45)
        stain_type = random.choice(['horiz', 'vert'])
        if stain_type == 'horiz':
            rx, ry = random.randint(200, 350), random.randint(6, 12)
            draw.ellipse([tx-rx, cy-ry, tx+rx, cy+ry], fill=(255, 255, 255))
        else:
            rx, ry = random.randint(15, 25), random.randint(40, 60)
            draw.ellipse([tx-rx, cy-ry, tx+rx, cy+ry], fill=(255, 255, 255))
    # 2. Large blobs
    for _ in range(random.randint(5, 8)):
        cx, cy = random.randint(X_MARGIN, text_limit_x), random.randint(0, 64)
        r = random.randint(15, 30)
        draw.ellipse([cx-r, cy-r, cx+r, cy+r], fill=(255, 255, 255))

# ── 4. Main Loop (Strict Sequential Logic) ──────────
    ...
    while word_idx < len(all_words):
        ...
        while word_idx < len(all_words):
            ...
            if not word_successfully_built:
                word_idx += 1 # Skip word if it can't be rendered
                continue

            # ROLL-OVER: If word doesn't fit, it stays at current word_idx for the next image
        ...
        # Apply Large Physical Stains
        apply_large_solid_stains(canvas, curr_x)

        name = f"large_solid_{img_count:05d}"
        ...
        img_count += 1
        # Set this to 1000 for your full Bucket 2 Large-Stain run
        if img_count >= 1500: break 

    print(f"✅ Large Solid Stains complete. Folder: {OUTPUT_DIR}")