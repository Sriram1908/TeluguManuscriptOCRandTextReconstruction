...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Type_4_Fade_Output")
...
# ── 3. Helper Functions (Sequence Shielded) ────────
...
def extract_real(img_path, h):
    ...
    # Tiny blur to make fading look like real weathered ink
...
def multiply_paste_faded(base_rgb, char_img, x, y, opacity):
    ...
    # Grainy Alpha logic for realistic ink wear
...
if __name__ == "__main__":
    ...
    while word_idx < len(all_words):
        ...
        while word_idx < len(all_words):
            ...
            # Deep fading range for Bucket 4 Pure
            word_opacity = random.uniform(0.12, 0.38)
            
            for i in word_imgs:
                multiply_paste_faded(canvas, i, curr_x, BASELINE_Y-(i.size[1]//2), word_opacity)
                curr_x += i.size[0] + CHAR_SPACING
            
            line_text.append(word); curr_x += WORD_SPACING; word_idx += 1
            
        name = f"faded_pure_{img_count:05d}"
    ...
    print(f"✅ 500 Pure Faded complete. Folder: {OUTPUT_DIR}")