...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Type_3_Med_Output")
...
def apply_medium_solid_stains(canvas, text_limit_x):
    draw = ImageDraw.Draw(canvas)
    for _ in range(random.randint(2, 4)):
        tx = random.randint(X_MARGIN, text_limit_x)
        cy = random.randint(10, 55)
        stain_type = random.choice(['horiz', 'vert', 'slit'])
        if stain_type == 'horiz':
            rx, ry = random.randint(40, 110), random.randint(3, 6)
            draw.ellipse([tx-rx, cy-ry, tx+rx, cy+ry], fill=(255, 255, 255))
        elif stain_type == 'vert':
            rx, ry = random.randint(5, 10), random.randint(20, 40)
            draw.ellipse([tx-rx, cy-ry, tx+rx, cy+ry], fill=(255, 255, 255))
        else:
            rx = random.randint(40, 80); skew = random.randint(-10, 10)
            draw.line([(tx-rx, cy-skew), (tx+rx, cy+skew)], fill=(255, 255, 255), width=random.randint(5, 8))
    ...
    while word_idx < len(all_words):
        ...
        while word_idx < len(all_words):
            ...
            for c in clusters:
                ...
                if img is None: img = generate_synthetic(c, syn_font, FIXED_HEIGHT)
                if img:
                    word_imgs.append(img); word_w += img.size[0] + CHAR_SPACING
            if curr_x + word_w > bg_rgb.shape[1]-RIGHT_MARGIN: break
            for i in word_imgs:
                multiply_paste_rgb(canvas, i, curr_x, BASELINE_Y-(i.size[1]//2))
                curr_x += i.size[0] + CHAR_SPACING
            line_text.append(word); curr_x += WORD_SPACING; word_idx += 1
            
        apply_medium_solid_stains(canvas, curr_x)
        name = f"med_solid_{img_count:05d}"
